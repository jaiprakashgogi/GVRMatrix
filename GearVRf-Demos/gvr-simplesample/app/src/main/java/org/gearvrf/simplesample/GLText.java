...
import org.gearvrf.simplesample.programs.BatchTextProgram;
import org.gearvrf.simplesample.programs.Program;
...
public class GLText {
	...
	public void draw(String text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ) {
		...
		for (int i = 0; i < len; i++) { // FOR Each Character in String
			...
			batch.drawSprite(letterX, letterY, chrWidth, chrHeight, charRgn[c], modelMatrix); // Draw
																								// the
																								// Character
			letterY -= (charHeight + spaceX) * scaleX; // Advance X Position by
														// Scaled Character
														// Width
		}
	}

	public void draw(char text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ) {
		float chrHeight = cellHeight * scaleY; // Calculate Scaled Character
												// Height
		float chrWidth = cellWidth * scaleX; // Calculate Scaled Character Width
												// // Get String Length
		...
		int c = (int) text - CHAR_START; // Calculate Character Index (Offset by
											// First Char in Font)
		if (c < 0 || c >= CHAR_CNT) // IF Character Not In Font
			c = CHAR_UNKNOWN; // Set to Unknown Character Index
		// TODO: optimize - applying the same model matrix to all the characters
		// in the string
		batch.drawSprite(letterX, letterY, chrWidth, chrHeight, charRgn[c], modelMatrix); // Draw
																							// the
																							// Character
	...
	public void draw(char text, float x, float y) {
		draw(text, x, y, 0, 0, 0, 0);
	}
	...
	public float drawC(String text, float x, float y, float z, float angleDegX, float angleDegY, float angleDegZ) {
		...
		draw(text, x - (len / 2.0f), y - (getCharHeight() / 2.0f), z, angleDegX, angleDegY, angleDegZ); // Draw
																										// Text
																										// Centered
	...
	public void drawTexture(int width, int height, float[] vpMatrix) {
		...
		batch.drawSprite(width - (textureSize / 2), height - (textureSize / 2), textureSize, textureSize, textureRgn,
				idMatrix); // Draw
		batch.endBatch(); // End Batch
	}
}
